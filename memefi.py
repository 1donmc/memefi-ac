...
proxies = {
    "http": "http://103.179.57.234:1000",
    "https": "http://103.179.57.234:1000"
}
    ...
    for attempt in range(retries):
        async with session.post(url, headers=headers, json=json_payload) as response:
            if response.status == 200:
                ...
                print(
                    f"❌ Gagal dengan status {response.status}, mencoba lagi ")
...
async def fetch(account_line):
    ...
    user_data = tg_web_data.split('user=', maxsplit=1)[
    ...
    url = 'https://api-gw-tg.memefi.club/graphql'
    # Membuat salinan headers_set agar tidak mengubah variabel global
        ...
        async with session.post(url, headers=headers, json=recharge_booster_payload) as response:
            if response.status == 200:
                ...
                if response_data and 'data' in response_data and response_data['data'] and 'telegramGameActivateBooster' in response_data['data']:
                    ...
                    print(
                        "❌ Gagal mengaktifkan Recharge Booster: Data tidak lengkap atau tidak ada.")
            else:
                # print(response)
                print(
                    f"❌ Gagal dengan status {response.status}, mencoba lagi..." + response)
                return None  # Mengembalikan None jika terjadi error
                        ...
                        if tap_result is not None:
                            if 'data' in tap_result and 'telegramGameProcessTapsBatch' in tap_result['data']:
                                ...
                                if tap_data['currentBoss']['currentHealth'] == 0:
                                    ...
                                    await set_next_boss(index, headers)
                                    print(
                                        f"\rTapped ✅ Coin: {tap_data['coinsAmount']}, Monster ⚔️: {tap_data['currentBoss']['currentHealth']} - {tap_data['currentBoss']['maxHealth']}    ")
                        else:
                            print(
                                f"❌ Gagal dengan status {tap_result}, mencoba lagi...")
            else:
                print(
                    f"❌ Gagal dengan status {response.status}, mencoba lagi...")
                return None  # Mengembalikan None jika terjadi error
...
async def check_and_complete_tasks(index, headers):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, json=task_list_payload, headers=headers) as response:
            ...
            all_completed = all(
                task['status'] == 'Completed' for task in tasks['data']['campaignTasks'])
            if all_completed:
                print(
                    f"\r[ Akun {index + 1} ] Semua tugas telah selesai. ✅            ", flush=True)
                return True
            ...
            for task in tasks['data']['campaignTasks']:
                ...
                if task['status'] == "Pending":
                    print(
                        f"\🔍 Viewing task: {task['name']}", end="", flush=True)
                    ...
                    async with session.post(url, json=view_task_payload, headers=headers) as view_response:
                        ...
                        if 'errors' in view_result:
                            print(
                                f"\r❌ Gagal mendapatkan detail task: {task['name']}")
                            print(view_result)
                        else:
                            task_details = view_result['data']['campaignTaskGetConfig']
                            print(
                                f"\r🔍 Detail Task: {task_details['name']}", end="", flush=True)

                    # Jeda 2 detik setelah melihat detail
                    await asyncio.sleep(2)

                    print(
                        f"\r🔍 Verifikasi task: {task['name']}                                                                ", end="", flush=True)
                    verify_task_payload = {
                    ...
                    async with session.post(url, json=verify_task_payload, headers=headers) as verify_response:
                        ...
                        if 'errors' not in verify_result:
                            print(
                                f"\r✅ {task['name']} | Moved to Verification", flush=True)
                        else:
                            print(
                                f"\r❌ {task['name']} | Failed to move to Verification", flush=True)
                            print(verify_result)
            ...
            async with session.post(url, json=task_list_payload, headers=headers) as response:
                ...
                for task in updated_tasks['data']['campaignTasks']:
                    ...
                    if task['status'] == "Verification":
                        print(
                            f"\r🔥 Menyelesaikan task: {task['name']}", end="", flush=True)
                        ...
                        async with session.post(url, json=complete_task_payload, headers=headers) as complete_response:
                            ...
                            if 'errors' not in complete_result:
                                print(
                                    f"\r✅ {task['name']} | Completed                         ", flush=True)
                            else:
                                print(
                                    f"\r❌ {task['name']} | Failed to complete            ", flush=True)

                    # Jeda 3 detik setelah menyelesaikan tugas
    ...
    while True:
        ...
        for index, line in enumerate(lines):
            ...
            if result is not None:
                ...
                print(
                    f"❌ Akun {index + 1}: Token tidak valid atau terjadi kesalahan")
        ...
        for index, result, first_name, last_name, league in accounts:
            ...
            print(
                f"\r[ Akun {index + 1} ] {first_name} {last_name} memeriksa task...", end="", flush=True)
                ...
                if energy_sekarang < 0.25 * user_data['maxEnergy']:
                    if auto_booster == 'y':
                        if user_data['freeBoosts']['currentRefillEnergyAmount'] > 0:
                            print(
                                "\r🪫 Energy Habis, mengaktifkan Recharge Booster... \n", end="", flush=True)
                            await activate_energy_recharge_booster(index, headers)
                            continue  # Lanjutkan tapping setelah recharge
                        else:
                            print(
                                "\r🪫 Energy Habis, tidak ada booster tersedia. Beralih ke akun berikutnya.\n", flush=True)

                    else:
                        print(
                            "\r🪫 Energy Habis, auto booster disable. Beralih ke akun berikutnya.\n", flush=True)
...
def animate_energy_recharge(duration):
    ...
    while time.time() < end_time:
        ...
        for frame in frames:
            print(
                f"\r🪫 Mengisi ulang energi {frame} - Tersisa {remaining_time} detik         ", end="", flush=True)
            time.sleep(0.25)
...
while True:
    auto_booster = input(
        "Use Energy Booster (default n) ? (y/n): ").strip().lower()
...
while True:
    turbo_booster = input(
        "Use Turbo Booster (default n) ? (y/n): ").strip().lower()
...
if turbo_booster == 'y':
    while True:
        god_mode = input(
            "Activate God Mode (1x tap monster dead) ? (y/n): ").strip().lower()
        if god_mode in ['y', 'n', '']:
